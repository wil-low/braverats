...
const char *SHORT_NAMES[] = {"Je", "Ps", "Sp", "As", "Mi", "Ma", "Ge", "PC"};
...
void render_player(GameState *state, uint8_t idx) {
    ...
    if (p->_level == Human)
        printf("Player %d (Human), score %d, effect %d:\n", idx, p->_score,
               p->_effect);
    else
        printf("Player %d (Lev.%d), score %d, effect %d:\n", idx, p->_level,
               p->_score, p->_effect);
...
void render_winner(GameState *state, RoundResult result) {
    ...
    if (result == RR_Hold)
        ...
        printf("Player %c wins! ", result == RR_Player0 ? '0' : '1');
...
void serialize_state(GameState *state, char *buf) {
    ...
    sprintf(
        ...
        (state->_players[0]._effect == EffectOpponentReveals), // P0_SpyEffect;"
        (state->_players[0]._effect == EffectStrength), // P0_GeneralEffect;"
        ...
        (state->_players[1]._effect == EffectOpponentReveals), // P1_SpyEffect;"
        (state->_players[1]._effect == EffectStrength), // P1_GeneralEffect;"
